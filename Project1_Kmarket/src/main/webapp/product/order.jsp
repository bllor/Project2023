...
<form action="#">
 ...
 <table>
     ...
     <tbody>
     	 <c:if test="${empty param.cartToOrders }">
         <tr class="empty">
             <td colspan="7">장바구니에 상품이 없습니다.</td>
         </tr>
         </c:if>
		<c:forEach var="cartToOrder" items="${paramValues.cartToOrders}">
		    <c:set var="cartInfo" value="${cartToOrder}" />
    		<c:set var="cartObj" value="${fn:split(cartInfo, ',')}" />
         <tr>
             <td>
                 <article>
                     <a href="${ctxPath }/product/view.do?prodNo=${cartObj[6]}&cate1=${cate1}&cate2=${cate2}">
                     	<img src="${paramValues.thumb2[status.cartToOrder]}" alt="${paramValues.thumb2[status.cartToOrder]}">
                     </a>
                     <div>
                         <h2><a href="${ctxPath }/product/view.do?prodNo=${cartObj[6]}&cate1=${cate1}&cate2=${cate2}">${paramValues.prodNames[status.cartToOrder]}<</a></h2>
                         <p>${paramValues.descript[status.cartToOrder]}</p>
                     </div>
                 </article>
             </td>
             <td>${cartObj[0]}</td> <!-- count -->
             <td>${cartObj[1]}</td> <!-- price -->
             <td>${cartObj[3]}</td> <!-- delivery -->
             <td>${cartObj[5]}</td> <!-- total -->
         </tr>
        </c:forEach>
         <tr>
             <td>
     ...
     <div>
         ...
         <p>
             ...
             <label>
                 ...
                 <img src="${ctxPath }/img/ico_kakaopay.gif" alt="카카오페이">
             </label>
         </p>
     </div>
 </article>
